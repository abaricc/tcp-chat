...
const char* do_echo(int client_id, char *msg, char *resp, int resp_len) {
  return "this is echo";
}
const char* do_rand(int client_id, char *msg, char *resp, int resp_len) {
  return "this is rand";
}
const char* do_quit(int client_id, char *msg, char *resp, int resp_len) {
  return "this is quit";
}

const char* eval_msg(int client_id, char *msg, char *resp, int resp_len) {
  char *cmd, *args;
  cmd = strsep(&msg," ");
  args = msg;
...
char* receive_message(int sock, char *buffer, int size) {
    char received[100];
    ...
    printf("Client %d => %s", sock, received);
...
void* client_main(void* arg) {
  ...
  int client_id = client_sock;
  char cmd[MESSAGE_MAXLEN];
  char response[MESSAGE_MAXLEN];
  char received[MESSAGE_MAXLEN];
  printf("Le client %d est arrive\n", client_sock);
  while(1) {
    //if(receive_message(client_sock, cmd, sizeof(cmd))<0)break;/*erreur:ondeconnecteleclient*/
    strcpy(received, receive_message(client_sock, cmd, sizeof(cmd))<0);
    printf("received = %s\n", received);
    strcpy(response, eval_msg(client_id, cmd, response, sizeof(response)));
    //if(<0) break;/*erreur:ondeconnecteleclient*/
...
int main(int argc, char* argv[]) {
  ...
  while(1) {
    ...
    if(client_arrived(client_sock)<0) {
      printf("Le client %d n'a pas pu se connecter\n", client_sock);
    }
